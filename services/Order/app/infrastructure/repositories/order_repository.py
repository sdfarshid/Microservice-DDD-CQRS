...
from sqlalchemy import select, update, delete, Row, RowMapping, insert
...
class OrderRepository(IOrderRepository):
    ...
    async def save(self, order: Order) -> Order:
        order_db = OrderMapper.to_order_orm(order)

        async def operation():
            await self.db.merge(order_db)
    ...
    async def update_order_item(self, item_id: UUID, status: str) -> OrderItem:
        async def operation():
            ...
            return OrderMapper.to_order_item_domain(row[0])
    ...
    async def add_order_items_batch(self, order_id: UUID, order_items: List[OrderItem]) -> None:
        values = [
            OrderMapper.to_order_item_dict(item, order_id)
            for item in order_items
        ]

        async def operation():
            stmt = insert(OrderItemDBModel).values(values)
    ...
    async def get_orders_by_ids(self, order_id: UUID) -> Order | None:
        ...
        order_db = result.unique().scalars().one_or_none()
